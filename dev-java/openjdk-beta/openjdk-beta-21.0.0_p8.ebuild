...
COMMON_DEPEND="
	...
	clang? (>=sys-deve/clang-14.0.6-r1)
"
...
DEPEND="
	...
	x11-libs/libXt
	x11-libs/libXtst
	javafx? ( dev-java/openjfx-beta:${SLOT}= )
	jtreg? ( >=dev-java/jtreg-beta-6.1.0_p2 )
	cache? ( dev-util/ccache )
	system-bootstrap? (
		|| (
			dev-java/openjdk-beta-bin:${SLOT}
			dev-java/openjdk-beta:${SLOT}
		)
	)
"
...
src_configure() {
	if has_version dev-java/openjdk-beta:${SLOT}; then
		export JDK_HOME=${BROOT}/usr/$(get_libdir)/openjdk-beta-${SLOT}
	elif use !system-bootstrap ; then
		local bootvar="${ARCH^^}_BOOT"
		export JDK_HOME="${WORKDIR}/${!bootvar}"
	else
		JDK_HOME=$(best_version -b dev-java/openjdk-beta-bin:${SLOT})
	...
	if use javafx; then
		local zip="${EPREFIX}/usr/$(get_libdir)/openjfx-beta-${SLOT}/javafx-exports.zip"
		if [[ -r ${zip} ]]; then
